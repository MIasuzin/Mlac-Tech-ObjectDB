...
Удаляет виртуальный каталог внутри базы.
...
Возвращает `true`, если каталог существовал и был удалён, и `false`, если каталога уже нет.

Удаляется только указанный каталог. Корневой каталог удалить нельзя.

---

### `db.has(path)`

Проверяет наличие JSON-документа без чтения и разбора его содержимого.

```js
db.has('users/1.json');
```

Возвращает `true`, если документ существует, и `false`, если документа нет или он был удалён.

В отличие от `read()`, метод не выполняет `JSON.parse()` и подходит для быстрой проверки существования данных.
