...
const {headerSize, headerStateOffset, metadataOffsets, metadataSize, headerStates, bucketCount, bucketSize, indexOffset, dataOffset, recordTypes, maxTransactionBytes, maxJournalTransactionRecords, previousVersion} = require('./constants.js');
...
function inspectTransaction(fd, startOffset, begin, fileSize, rebuiltIndex, livePaths, liveGroups) {
  ...
  if (count === 0 || count > maxJournalTransactionRecords || mutationBytes > maxTransactionBytes) {
...
function validateMutation(mutation, offset, previous) {
  ...
  if ((mutation.type === recordTypes.DIRECTORY && previous?.type === recordTypes.WRITE) ||
      (mutation.type === recordTypes.WRITE && previous?.type === recordTypes.DIRECTORY) ||
            (mutation.type === recordTypes.DELETE && previous?.type !== recordTypes.WRITE && previous?.type !== recordTypes.DIRECTORY)) {
