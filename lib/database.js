...
class Database {
  ...
  *walkBucket(bucketIndex) {
  ...
  readBucket(bucketIndex) {
    ...
    if (!buffer.equals(this.indexBuffer.subarray(bucketIndex * bucketSize, (bucketIndex + 1) * bucketSize))) {
      throw createError(codes.corruptedIndex, `Index bucket ${bucketIndex} differs from the opened state`);
    }
