...
function createWriterLockFile(lockPath) {
  ...
  if (identity !== null) info.identity = identity;
...
function getProcessIdentity(pid) {
  try {
    ...
    if (process.platform === 'win32') {
      ...
      const powershellPath = require('path').win32.join(process.env.SystemRoot || process.env.windir || 'C:\\Windows', 'System32', 'WindowsPowerShell', 'v1.0', 'powershell.exe');
      const result = execFileSync(powershellPath, ['-NoProfile', '-NonInteractive', '-Command', command], {
        encoding: 'utf8',
        windowsHide: true,
        timeout: 5000,
