  ...
  try {
    ...
    if (current) {
      if (isProcessAlive(current.pid)) {
        throw createError(codes.alreadyOpen, `Database is already open by process ${current.pid}: ${filePath}`);
      }
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      ...
      if (error.code === codes.corruptedLock) {
        ...
        throw createError(codes.lockGuardExists, `Writer lock creation is already in progress: ${guardPath}`);
      }
