...
Здесь описан текущий код до назначения версии следующей публикации. Версия формата `.mtdb` (`3`) и версия npm-пакета — разные числа. Последний приложенный `package.json` указывает `0.3.0-rc.2`; дату и номер будущего выпуска этот документ не предполагает.
